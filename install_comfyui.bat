...
if (Test-Path $pythonPath) {
    ...
    if (Test-Path $reqsPath) {
        ...
        try {
            $proc = Start-Process -FilePath $pythonPath -ArgumentList "-m pip install -r `"$reqsPath`"" -WorkingDirectory $installDir -PassThru -NoNewWindow -Wait
    ...
    # 4C. Create 'run_with_manager.bat' (Explicit Request)
    $mgrBatPath = Join-Path $installDir "run_with_manager.bat"
    $mgrBatContent = @"
.\python_embeded\python.exe -m pip install -r ComfyUI\manager_requirements.txt
.\python_embeded\python.exe -s ComfyUI\main.py --windows-standalone-build --enable-manager
pause
"@
    Set-Content -Path $mgrBatPath -Value $mgrBatContent
    Write-Success "Created 'run_with_manager.bat' in installation folder."
